window.__mc4gd_serializer = true;
// serializer.js — Rôle : readability-content-extractor
// VERSION MC4GD : Extraction Readability → Container autonome → Tainted Canvas Protection
//
...
// 6. Retourner un container 100% autonome, prêt pour jsPDF ou markdown
//
// Ce module ne "convertit" rien en PDF/MD. Il prépare le DOM.

window.MC4GDSerializer = {
  ...
  async process(wrapperClone) {
    ...
    if (article && article.content && article.content.length > 200) {
      ...
      console.warn('[MC4GD] Readability: échec ou contenu insuffisant — fallback DOM complet.');
  ...
  _tryReadability() {
    try {
      if (typeof Readability === 'undefined') {
        console.warn('[MC4GD] Readability.js non chargé — injection manquante ?');
        return null;
      }
      ...
      console.warn('[MC4GD] Erreur Readability:', e.message);
      return null;
    }
  },
  ...
  _buildContainer(contentHtml, title, byline, siteName) {
    ...
    labelDiv.textContent = 'Métadonnées de Capture (Google Drive)';
  ...
  async _protectTaintedCanvas(container) {
    ...
    const promises = Array.from(images).map(async (img) => {
      ...
      try {
        ...
        console.warn('[MC4GD] Image non convertible:', absoluteUrl, err.message);
        img.remove();
        failed++;
      }
    });
